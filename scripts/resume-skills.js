...
function skills_updareGroups(group=0) {
	skills_selectedGroup = group;
	...
	skills_bubbles.attr("fill", function(d) { return skills_colorScale(d.group[skills_selectedGroup]); });
 ...
 */
export function initialize_skills() {
	...
	menu.listen("MDCMenu:selected", (event) => {
		...
		// Call your function to regroup the skills with the selected index
		skills_updareGroups(selectedIndex);
	});
	...
	skills_updateWords();
	skills_updareGroups(); // Calls skills_updateDimensions;
}